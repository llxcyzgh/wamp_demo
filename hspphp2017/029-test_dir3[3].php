...
{
    if (is_dir($dir_src)) {
        $size = real($dir_src, $dir_des);
        echo $dir_src . '&nbsp;&nbsp;&nbsp;&nbsp;是个目录[已完成复制]<br>';
        echo $dir_src . '&nbsp;&nbsp;&nbsp;&nbsp;该目录大小为 '.$size.'字节, 也就是 '.round(($size/1024/1024),2).' MB<br>';
...
function real($dir_src, $dir_des)
{
    ...
    foreach ($arr as $value) {
        ...
        if (is_dir($dir_src . DS . $value)) {
            $size += real($dir_src . DS . $value, $dir_des . DS . $value);
        } elseif (is_file($dir_src . DS . $value)) {
            copy($dir_src . DS . $value, $dir_des . DS . $value);
            $size += filesize($dir_src . DS . $value);
        }
    }
